	...
	const string& newProductDescription)
{
	...
	sellingPriceDouble = parseSellingPrice(newSellingPrice);
}

double Product::parseSellingPrice(const std::string& priceStr) {
	double result = 0;
	try {
		string cleanPrice;
		for (char c : priceStr) {
		...
		result = stod(cleanPrice);
	}
	catch (const std::exception& error) {
		//cout << "Error converting sellingPrice to double: " << error.what() << endl;
		//cout << this->getUniqueID() << endl;
	}
	return result;
}
...
double Product::getSellingPriceDouble() const {
...
void Product::displayIDandName(ostream& out) {
	...
	// Debug printing: displays the selling price
	//out << "Selling Price String: " << this->getSellingPrice() << "\n";
	//out << "Selling Price Double: " << this->getSellingPriceDouble() << "\n" << endl;
}

// Overloaded greater than operator for sorting
bool Product::operator>(const Product& other) const {
	return this->sellingPriceDouble > other.getSellingPriceDouble();
}
...
// Overloaded less than or equal to operator for sorting
bool Product::operator<=(const Product& other) const {
	return this->sellingPriceDouble <= other.getSellingPriceDouble();
}