...
void ProductInventoryQuery::printHelp()
{
	...
	cout << " 2. listInventory <category_string> <method> <order> - Lists just the id and name of all inventory belonging to the specified category in order of selling price.";
	cout << "If no method is specified, defaults to using insertion sort. If method specified is \"merge\", uses merge sort. If no order is specified, defaults to ascending order.";
	cout << "If order is \"desc\" then order is descending. If the category doesn't exists, prints 'Invalid Category'.\n"
		<< endl;
}
...
// Locates inventory matching a target category, sorts using merge or insertion sort depending on user input,
//		and displays the result.
void ProductInventoryQuery::listInventory(string& targetID) {
	//mInventoryByCategory->listMatchingTargets(cout, targetID);
	bool descending = (targetID.rfind("desc") != -1);
	if (targetID.rfind("merge") != -1) {
		cleanCategoryString(targetID);
		mInventoryByCategory->mergeSortAndListMatchingTargets(cout, targetID, descending);
	}
	else {
		cleanCategoryString(targetID);
		mInventoryByCategory->insertionSortAndListMatchingTargets(cout, targetID, descending);
	}
}

// Removes format specifiers "merge" and "desc" from potential category string
void ProductInventoryQuery::cleanCategoryString(string& category) {
	string merge = " merge", desc = " desc";
	size_t found = category.rfind(merge);
	if (found != std::string::npos) {
		category.replace(found, merge.length(), "");
	}
	found = category.rfind(desc);
	if (found != std::string::npos) {
		category.replace(found, desc.length(), "");
	}
}