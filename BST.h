...
protected:
	...
	void printLevelOrderHelper(Node<T>* root, std::ostream& out) {
		if (root != nullptr) {
			queue<Node<T>*> printQueue;
			printQueue.push(root);

			while (!printQueue.empty()) {
				Node<T>* pCurrent = printQueue.front();

				if (pCurrent->left != nullptr) {
					printQueue.push(pCurrent->left);
				}
				if (pCurrent->right != nullptr) {
					printQueue.push(pCurrent->right);
				}
...
public:
	...
	/* MA: Implemented by calling BST.makeEmpty() */
	~BST() {
		if (this->_debug) {
	...
	/* MA: Implemented by calling cloneTree() */
